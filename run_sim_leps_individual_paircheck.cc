...
int main() {
    ...
    TFile* outFile = new TFile("sim_tuples_leps_individual_paircheck.root", "RECREATE");
    ...
    for (int i = 0; i < nBins; ++i) {
        ...
        qmTuples[i] = new TNtuple("quark_muon", "quark_muon", "binTag:event:parentFlag:pairFlag:pairFlagConf:ptHat:idQ:idL:ptQ:ptL:etaQ:etaL:phiQ:phiL");        
        qeTuples[i] = new TNtuple("quark_elec", "quark_elec", "binTag:event:parentFlag:pairFlag:pairFlagConf:ptHat:idQ:idL:ptQ:ptL:etaQ:etaL:phiQ:phiL");        
            ...
            if(121<=code && code<=124) //only checks quarks if they come from a hardest process which produces a heavy flavor pair
            {
                for (int i = 0; i < pythia.event.size(); ++i) {
                    ...
                    if (particleStatus==-23)
                    {
                        ...
                        {
                            if(std::abs(pythia.event[child].id())==11){quarkElecPos.push_back({(double)child, (double)pythia.event[child].id(), (double)particleID, particlePt, particleEta, particlePhi});}
                            if(std::abs(pythia.event[child].id())==13){quarkMuonPos.push_back({(double)child, (double)pythia.event[child].id(), (double)particleID, particlePt, particleEta, particlePhi});}
                        }
                    ...
                    if(particleStatus>0 && std::abs(particleID)==11)
                    {
                        ...
                        for(vector<double> child: quarkElecPos)
                        {
                            if((int)child[0]==i)
                            {
                                ...
                                if (!quarkMuonPos.empty()) 
                                {
                                    muonFlag=1;
                                    for(vector<double> muon: quarkMuonPos)
                                    {
                                        if((int)muon[1]*particleID<1 && (int)muon[2]*parentID<1){muonConfFlag=1;}
                                    }
                        ...
                        //binTag:event:parentFlag:pairFlag:pairFlagConf:ptHat:idQ:idL:ptQ:ptL:etaQ:etaL:phiQ:phiL
                        qeTuples[iBin]->Fill(iBin,iEvent,quarkFlagElec,muonFlag,muonConfFlag,pTHat,parentID,particleID,parentPt,particlePt,parentEta,particleEta,parentPhi,particlePhi);
                    }
                    ...
                    if(particleStatus>0 && std::abs(particleID)==13)
                    {
                        ...
                        for(vector<double> child: quarkMuonPos)
                        {
                            if((int)child[0]==i)
                            {
                                ...
                                if (!quarkElecPos.empty()) 
                                {
                                    elecFlag=1;
                                    for(vector<double> elec: quarkElecPos)
                                    {
                                        if((int)elec[1]*particleID<1 && (int)elec[2]*parentID<1){elecConfFlag=1;}
                                    }
                        ...
                        qmTuples[iBin]->Fill(iBin,iEvent,quarkFlagMuon,elecFlag,elecConfFlag,pTHat,parentID,particleID,parentPt,particlePt,parentEta,particleEta,parentPhi,particlePhi);
                    }
                }
            }
        }
